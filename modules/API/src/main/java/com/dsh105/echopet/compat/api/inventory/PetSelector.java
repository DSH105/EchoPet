...
package com.dsh105.echopet.compat.api.inventory;
...
public class PetSelector {
    ...
    public static Layout getDefaultLayout() {
        ...
        for (int j = 1; j < 10; j++) {
            ...
            if (preset != null) {
                layout.setSlot(45 - j, new CommandIcon(preset.getCommand(), preset.getMaterial(), 1, preset.getMaterialData(), preset.getName()));
            }
        }
        return layout;
    }
    ...
    public static Menu prepare() {
        return getLayout().toMenu(EchoPet.getPlugin());
    }
}